...
#define _TF_LOG_INFO \
  ::internal::LogMessage(__FILE__, __LINE__, ::INFO)
#define _TF_LOG_WARNING \
  ::internal::LogMessage(__FILE__, __LINE__, ::WARNING)
#define _TF_LOG_ERROR \
  ::internal::LogMessage(__FILE__, __LINE__, ::ERROR)
#define _TF_LOG_FATAL \
  ::internal::LogMessageFatal(__FILE__, __LINE__)
...
#define VLOG_IS_ON(lvl)                                                     \
  (([](int level, const char* fname) {                                      \
    static const bool vmodule_activated =                                   \
        ::internal::LogMessage::VmoduleActivated(fname, level); \
...
#define VLOG(level)                                              \
  ...
  ? (void)0                                                      \
  : ::internal::Voidifier() &                        \
          ::internal::LogMessage(__FILE__, __LINE__, \
                                             ::INFO)
...
#define DVLOG(verbose_level) \
  while (false && (verbose_level) > 0) ::internal::LogMessageNull()
#endif
...
#define LOGGING_INTERNAL_STATEFUL_CONDITION(kind, condition, arg)   \
  for (bool logging_internal_stateful_condition_do_log(condition);  \
    ...
    for (static ::internal::Log##kind##State            \
...
#define CHECK_OP_LOG(name, op, val1, val2)                            \
  while (::internal::CheckOpString _result =              \
             ::internal::name##Impl(                      \
                 ::internal::GetReferenceableValue(val1), \
                 ::internal::GetReferenceableValue(val2), \
                 #val1 " " #op " " #val2))                            \
...
#define CHECK_NOTNULL(val)                                 \
  ::internal::CheckNotNull(__FILE__, __LINE__, \
